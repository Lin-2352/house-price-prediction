...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\textwidth]{figures/app_prediction.png}
\caption{A live prediction on the deployed application, showing the predicted price
and the 90\% range for a sample Bangalore flat. The full result view (not pictured)
also carries a low-confidence check and a per-feature explanation chart directly
beneath this panel.}
\label{fig:appresult}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.3\textwidth]{figures/app_metrics.png}
\caption{The headline error metric from the deployed application's ``Model
performance \& validation'' panel, matching Table~\ref{tab:point}'s Mean Absolute
Error. The full panel also reproduces the coverage-by-method numbers from
Table~\ref{tab:coverage}, surfaced directly to the user rather than left in an
offline report only.}
