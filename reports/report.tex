...
\begin{figure}[H]
\centering
\includegraphics[width=0.62\textwidth]{figures/app_prediction.png}
\caption{A live prediction on the deployed application, showing the predicted price,
the full 90\% range, the low-confidence check, and most of the per-feature
explanation chart for a sample Bangalore flat.}
\label{fig:appresult}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{figures/app_metrics.png}
\caption{The deployed application's ``Model performance \& validation'' panel,
showing the point-accuracy metrics from Table~\ref{tab:point} and the
coverage-by-method table from Table~\ref{tab:coverage}, surfaced directly to the
user rather than left in an offline report only.}
